...
create_iqr <- function(samples, label, band) {
  band_tb <- sits::sits_select(samples[samples$label == label,], band)
  ...
  data.table::setnames(dt_mean, "V1", "Mean")
...
stats_ts <- function(ts_tibble) {
  ...
  labels <- sits::sits_labels(ts_tibble)
  bands <- sits::sits_bands(ts_tibble)
...
inter_class_correlation <- function(ts_tibble){
  bands <- sits::sits_bands(ts_tibble)
  pattern_classes <- ts_tibble$label
  ...
  for (b in bands){
    for (x in 1:ncol(comb_2)){
      pattern1 <- ts_tibble[ts_tibble$label == comb_2[,x][1],]$time_series[[1]][b]
      pattern2 <-ts_tibble[ts_tibble$label == comb_2[,x][2],]$time_series[[1]][b]
      df_corr <- data.frame(
...
intra_class_correlation <- function(ts_tibble){
